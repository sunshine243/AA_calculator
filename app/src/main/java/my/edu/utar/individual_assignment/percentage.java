...
public class percentage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button bt_continues = findViewById(R.id.btn_continues);
        Button bt_calculate = findViewById(R.id.btn_calculate);
        Button bt_back = findViewById(R.id.back);
        EditText totalNumPerson = findViewById(R.id.et_tp);
                    ...
                    try {
                        ...
                        try {
                            for (int i = 0; i < display.size(); i++) {
                                t_percentage += display.get(i).getNumber();
                            }
                            if (t_percentage == 100) {
                                for (int i = 0; i < display.size(); i++) {
                                    double result = totalAmount * (display.get(i).getNumber() / 100);
                                ...
                                Toast.makeText(percentage.this, "Percentage not equal 100%", Toast.LENGTH_SHORT).show();
                            }
                        } catch (NumberFormatException e) {
                        ...
                        if (et2View instanceof EditText) {
                            ...
                            if (!name.isEmpty() && !input.isEmpty()) {
                                try {
                                    ...
                                    display.add(new display(name, inputValue, 2));
                                }catch (NumberFormatException e){
